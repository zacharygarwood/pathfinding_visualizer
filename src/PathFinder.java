...
public class PathFinder {
    ...
    //list closed points and of all walls on the grid
    ...
    public void aStarPath() {
        ...
        for(int i = 0; i < 3; i++) {
            for(int j = 0; j < 3; j++) {
                ...
                if(xCoord < 0 || yCoord < 0 || xCoord >= control.getWidth() || yCoord >= control.getHeight()) {
                    continue;
                }
                ...
                if(isWall(new Point(wallJumpX, current.getY())) || isWall(new Point(current.getX(), wallJumpY)) && ((j == 0 | j == 2) && i != 1)) {
                    continue;
                }
